...
func writeNtpServer(core Core) {
	...
	if ntpServer.IpMode == "IPv4" {
		if read_reg(core.BaseAddrLReg+ntpServer.ConfigIpReg, &temp_data) == 0 {
		...
		if read_reg(core.BaseAddrLReg+ntpServer.ConfigIpReg, &temp_data) == 0 {
			...
			if read_reg(core.BaseAddrLReg+ntpServer.ConfigIpv61Reg, &temp_data) == 0 {
				...
				if read_reg(core.BaseAddrLReg+ntpServer.ConfigIpv62Reg, &temp_data) == 0 {
					...
					if read_reg(core.BaseAddrLReg+ntpServer.ConfigIpv63Reg, &temp_data) == 0 {
	...
	if write_reg(core.BaseAddrLReg+ntpServer.UtcInfoControlReg, &temp_data) == 0 {
		for i := range 10 {
			if read_reg(core.BaseAddrLReg+ntpServer.UtcInfoControlReg, &temp_data) == 0 {
				if (temp_data & 0x80000000) != 0 {
					if read_reg(core.BaseAddrLReg+ntpServer.UtcInfoReg, &temp_data) == 0 {
	...
	// status
	if read_reg(core.BaseAddrLReg+ntpServer.CountReqReg, &temp_data) == 0 {
		ntpServer.RequestsValue = fmt.Sprintf("%d", temp_data)
	} else {
		ntpServer.RequestsValue = "NA"
	}

	if read_reg(core.BaseAddrLReg+ntpServer.CountRespReg, &temp_data) == 0 {
	...
	if read_reg(core.BaseAddrLReg+ntpServer.CountReqDroppedReg, &temp_data) == 0 {
		ntpServer.RequestsDroppedValue = fmt.Sprintf("%d", temp_data)
	} else {
		ntpServer.RequestsDroppedValue = "NA"
	}

	if read_reg(core.BaseAddrLReg+ntpServer.CountBroadcastReg, &temp_data) == 0 {
	...
	if read_reg(core.BaseAddrLReg+ntpServer.CountControlReg, &temp_data) == 0 {
		if (temp_data & 0x00000001) == 0 {
	...
	// version
	if read_reg(core.BaseAddrLReg+ntpServer.VersionReg, &temp_data) == 0 {
